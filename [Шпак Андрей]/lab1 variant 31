...
int main()
{
	...
	if (sum > 255)						//Может произойти так, что сумма превышает диапазон byte, для этого существует данная проверка	
	{
		sum = abs(sum - j - i);			//Воспользуемся следующей формулой, в которой вычисляется нужное по условию задачи значение sum, оно и будет числом, не превышающим диапазон byte			
		i = 1;							//Присваиваем новые значения счётчикам для того, чтобы выполнить следующую задачу 
		j = 1;
	}
	printf("Номер последнего числа Фибоначчи, которое входит в диапазон byte: ");
	printf("%d", sum);
	...
	printf("\nПроизведение первых чисел Фибоначчи, таких, что значение произведения не превышает диапазон int: ");
	printf("%d", mult);
	printf("\n");
	system("pause");
	return 0;
